...
from utils.helper_tool import ConfigToronto3D as cfg
...
class Toronto3D:
    def __init__(self, mode='train'):
        ...
        self.train_files = [os.path.join(
            self.full_pc_folder, files + '.ply') for files in self.train_files]
        self.val_files = [os.path.join(
            self.full_pc_folder, files + '.ply') for files in self.val_files]
        self.test_files = [os.path.join(
            self.full_pc_folder, files + '.ply') for files in self.test_files]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='train',
                        help='options: train, test, vis')
    parser.add_argument('--model_path', type=str,
                        default=None, help='pretrained model path')
    parser.add_argument('--test_eval', type=bool, default=True,
                        help='evaluate test result on L002')
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                Plot.draw_pc_sem_ins(
                    sub_pc_xyz[0, :, :], labels[0, 0:np.shape(sub_pc_xyz)[1]])
