...
from utils.helper_dp import DataProcessing as DP
...
class Network:
    def __init__(self, dataset, config):
        ...
        if self.config.saving:
            ...
            makedirs(self.saving_path) if not exists(
                self.saving_path) else None
        ...
        with tf.variable_scope('inputs'):
            ...
            self.Log_file = open(
                'log_train_' + dataset.name + str(dataset.val_split) + log_time + '.txt', 'a')
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss = self.get_loss(
                valid_logits, valid_labels, self.class_weights)
        ...
        with tf.variable_scope('results'):
            self.correct_prediction = tf.nn.in_top_k(
                valid_logits, valid_labels, 1)
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            # SCF
            f_encoder_i = self.scf_module(
                feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i], 'Encoder_layer_' + str(i), is_training)
            # RS
        ...
        feature = helper_tf_util.conv2d(f_encoder_list[-1], f_encoder_list[-1].get_shape()[
                                        3].value, [1, 1], 'decoder_0', [1, 1], 'VALID', True, is_training)
        ...
        f_layer_fc3 = helper_tf_util.conv2d(f_layer_drop, self.config.num_classes, [
                                            1, 1], 'fc', [1, 1], 'VALID', False, is_training, activation_fn=None)
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                if self.training_step % 50 == 0:
                    ...
                    log_out(message.format(self.training_step, l_out,
                            acc, 1000 * (t_end - t_start)), self.Log_file)
                self.training_step += 1
                ...
                if m_iou > np.max(self.mIou_list):
                    # Save the best model
                    self.saver.save(self.sess, snapshot_directory +
                                    '/snap', global_step=self.training_step)
                    self.saver.save(
                        self.sess, snapshot_directory + '/scf_best')
                self.mIou_list.append(m_iou)
                log_out('Best m_IoU is: {:5.3f}'.format(
                    max(self.mIou_list)), self.Log_file)

                # save latest model
                self.saver.save(self.sess, snapshot_directory + '/scf_latest')

                self.training_epoch += 1
                self.sess.run(dataset.train_init_op)

                # Update learning rate
                op = self.learning_rate.assign(tf.multiply(
        ...
        if m_iou > np.max(self.mIou_list):
            ...
            self.saver.save(self.sess, snapshot_directory +
                            '/snap', global_step=self.training_step)
            self.saver.save(self.sess, snapshot_directory + '/scf_best')
        self.mIou_list.append(m_iou)
        log_out('Best m_IoU is: {:5.3f}'.format(
            max(self.mIou_list)), self.Log_file)
    ...
    def evaluate(self, dataset):
            ...
            try:
                ...
                if not self.config.ignored_label_inds:
                    ...
                    invalid_idx = np.where(
                        labels == self.config.ignored_label_inds)[0]
        ...
        log_out('eval accuracy: {}'.format(
            val_total_correct / float(val_total_seen)), self.Log_file)
    ...
    def scf_module(self, feature, xyz, neigh_idx, d_out, name, is_training):
        ...
        f_lc, lg_volume_ratio = self.local_context_learning(
            xyz, f_pc, neigh_idx, d_out, name + 'LFA', is_training)
        # MLP 2
        f_lc = helper_tf_util.conv2d(f_lc, d_out * 2, [1, 1], name + 'mlp2', [
                                     1, 1], 'VALID', True, is_training, activation_fn=None)
        # MLP Shotcut
        shortcut = helper_tf_util.conv2d(feature, d_out * 2, [1, 1], name + 'shortcut', [
                                         1, 1], 'VALID', activation_fn=None, bn=True, is_training=is_training)

        # Global Contextual Features
        f_gc = tf.expand_dims(tf.concat([xyz, lg_volume_ratio], axis=-1), -2)
        f_gc = helper_tf_util.conv2d(f_gc, d_out * 2, [1, 1], name + 'lg', [
                                     1, 1], 'VALID', activation_fn=None, bn=True, is_training=is_training)
    ...
    def local_context_learning(self, xyz, feature, neigh_idx, d_out, name, is_training):
        ...
        # LPR
        local_rep, g_dis, lg_volume_ratio = self.local_polar_representation(
            xyz, neigh_idx)

        # 1
        local_rep = helper_tf_util.conv2d(
            local_rep, d_in, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        ...
        f_lc = self.dualdis_att_pool(
            f_concat, f_dis, g_dis, d_out // 2, name + 'dis_att_pooling_1', is_training)

        # 2
        local_rep = helper_tf_util.conv2d(
            local_rep, d_out // 2, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(
            tf.squeeze(f_lc, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, local_rep], axis=-1)
        f_dis = self.cal_feature_dis(tf.squeeze(f_lc, axis=2), f_neighbours)
        f_lc = self.dualdis_att_pool(
            f_concat, f_dis, g_dis, d_out, name + 'dis_att_pooling_2', is_training)
    ...
    def local_polar_representation(self, xyz, neigh_idx):
        ...
        relative_info, relative_alpha, relative_beta, geometric_dis, local_volume = self.relative_pos_transforming(
            xyz, neigh_idx, neighbor_xyz)

        # Local direction calculation (angle)
        neighbor_mean = tf.reduce_mean(neighbor_xyz, axis=-2)
        direction = xyz - neighbor_mean
        direction_tile = tf.tile(tf.expand_dims(direction, axis=2), [
                                 1, 1, tf.shape(neigh_idx)[-1], 1])

        direction_alpha = tf.expand_dims(
            tf.atan2(direction_tile[:, :, :, 1], direction_tile[:, :, :, 0]), axis=-1)
        direction_xydis = tf.sqrt(tf.reduce_sum(
            tf.square(direction_tile[:, :, :, :2]), axis=-1))
        direction_beta = tf.expand_dims(
            tf.atan2(direction_tile[:, :, :, 2], direction_xydis), axis=-1)
        ...
        global_dis = tf.sqrt(tf.reduce_sum(
            tf.square(xyz), axis=-1, keepdims=True))
    ...
    def relative_pos_transforming(self, xyz, neigh_idx, neighbor_xyz):
        ...
        relative_alpha = tf.expand_dims(
            tf.atan2(relative_xyz[:, :, :, 1], relative_xyz[:, :, :, 0]), axis=-1)
        relative_xydis = tf.sqrt(tf.reduce_sum(
            tf.square(relative_xyz[:, :, :, :2]), axis=-1))
        relative_beta = tf.expand_dims(
            tf.atan2(relative_xyz[:, :, :, 2], relative_xydis), axis=-1)
        relative_dis = tf.sqrt(tf.reduce_sum(
            tf.square(relative_xyz), axis=-1, keepdims=True))

        relative_info = tf.concat(
            [relative_dis, xyz_tile, neighbor_xyz], axis=-1)

        # negative exp of geometric distance
        exp_dis = tf.exp(-relative_dis)

        # volume of local region
        local_volume = tf.pow(tf.reduce_max(
            tf.reduce_max(relative_dis, -1), -1), 3)
    ...
    def cal_feature_dis(self, feature, f_neighbours):
        ...
        """
        feature_tile = tf.tile(tf.expand_dims(feature, axis=2), [
                               1, 1, tf.shape(f_neighbours)[-2], 1])
        feature_dist = feature_tile - f_neighbours
        feature_dist = tf.expand_dims(tf.reduce_mean(
            tf.abs(feature_dist), axis=-1), axis=-1)
    ...
    def dualdis_att_pool(feature_set, f_dis, g_dis, d_out, name, is_training):
        ...
        concat = tf.concat(
            [g_dis_reshaped, f_dis_reshaped, f_reshaped], axis=-1)

        # weight learning
        att_activation = tf.layers.dense(
            concat, d, activation=None, use_bias=False, name=name + 'dis_self_fc')
        ...
        # shared MLP
        f_lc = helper_tf_util.conv2d(
            f_lc, d_out, [1, 1], name + 'mlp', [1, 1], 'VALID', True, is_training)
