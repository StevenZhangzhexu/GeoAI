...
def separate_segmented_point_clouds_original(filename):
...
def process_segment(segment_id, segment_points, output_dir):
    coordinates = np.vstack((segment_points['x'], segment_points['y'], segment_points['z'])).T
    print(label_to_names[segment_id], len(coordinates))
    ...
    for cluster_label in unique_labels:
        ...
        # Save the cluster as a separate .laz file
        output_file = os.path.join(output_dir, f"segment_{segment_id}_{label_to_names[segment_id]}_object_{cluster_label}.laz")
        result.append((output_file, cluster_points))
...
def separate_segmented_point_clouds(filename):
    ...
    pool = Pool()  # Use multiprocessing for parallel processing

    # Collect results from all segments
    results = []
    for segment_id in segment_ids:
        segment_points = inFile.points[inFile.pred == segment_id]
        results.extend(pool.starmap(process_segment, [(segment_id, segment_points, output_dir)]))

    pool.close()
    pool.join()

    for result in results:
        for output_file, points in result:
            save_separate_laz_point_cloud(output_file, points)
