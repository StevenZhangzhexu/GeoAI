...
from utils.helper_ply import write_ply, read_ply
...
for pc_path in [join(original_pc_folder, fname + '.ply') for fname in train_files + val_files]:
    ...
    xyz = np.vstack((pc['x'] - UTM_OFFSET[0], pc['y'] - UTM_OFFSET[1],
                    pc['z'] - UTM_OFFSET[2])).T.astype(np.float32)
    ...
    sub_xyz, sub_colors, sub_labels = DP.grid_sub_sampling(
        xyz, color, labels, grid_size)
    _, sub_intensity = DP.grid_sub_sampling(
        xyz, features=intensity, grid_size=grid_size)
