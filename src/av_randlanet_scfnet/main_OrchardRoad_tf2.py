...
class OrchardRoad:
    def __init__(self, mode='train'):
        ...
        self.train_files = [os.path.join(self.full_pc_folder, files + '.las') for files in self.train_files]
        self.val_files = [os.path.join(self.full_pc_folder, files + '.las') for files in self.val_files]
        self.test_files = [os.path.join(self.full_pc_folder, files + '.las') for files in self.test_files]
    ...
    def get_tf_mapping(self):
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            batch_features = tf.map_fn(self.tf_augment_input, [batch_xyz, batch_features], fn_output_signature=tf.float32)
            input_points = []
            input_neighbors = []
            input_pools = []
            input_up_samples = []
